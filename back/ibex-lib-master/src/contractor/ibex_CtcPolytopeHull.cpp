...
#include "/app/src/contractor/ann.cpp"
...
void CtcPolytopeHull::contract(IntervalVector& box, ContractContext& context) {
	...
	try {
		...
		if((rand() % 100) <=50 && true){
				...
				Simplex_P.open("results/DS_P.txt",std::ios_base::app);
				...
				intervals_file.open("results/intervals_change.txt",std::ios_base::app);
				for (int i=0;i<diams.size();i++){intervals_file<< diams[i] << " ";}
				intervals_file<<endl;
				intervals_file.close();

				std::ofstream AXB;
				AXB.open("results/AXB.txt",std::ios_base::app);
				for (int i=0;i<lr2->L_A.nb_rows();i++){
					for (int k=0;k<lr2->L_A.nb_cols();k++){
						AXB<<lr2->L_A[i][k]<<" ";
					}
...
void CtcPolytopeHull::optimizer(IntervalVector& box, float proba) {
